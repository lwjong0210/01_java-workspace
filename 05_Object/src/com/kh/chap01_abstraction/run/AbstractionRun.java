...
public class AbstractionRun {
	 ...
	 *		   키	: double height; 
	 *
	 * 6. 위에 선정된 것들을 가지고 본격적으로 
	 * 	1) "변수" 만을 프로그래망을 한다면?
	 * 		* 변수 - 하나의 자료형의 하나의 값만을 보관할 수 있음
	 * 		문동은이라는 학생 객체를 하나 만들어내기 위해서? --> String name1 = "문동은"; int age1 = 20; double height = 162.3;
	 * 		박연진이라는 학생 객체를 하나 만들어내기 위해서? --> String name2 = "박연진"; int age2 = 21; double height = 173.4; 
	 * 		이사라이라는 학생 객체를 하나 만들어내기 위해서? --> String name3 = "박연진"; int age2 = 22; double height = 150.4;
	 * 		.... 등등의 수백명에 해당하는 학생들 만둘어야됨!
	 ...
	 * 		학생들의 키를 보관하는 double[] height = {162.3, 173.4, 150.4,...}
	 * 
	 * 		어떤문제가 발생??
	 * 		=> 기존의 학생 정보를 지워야 할 때 뒤의 정보들을 앞으로 땡기는 과정을 진행해야함..
	 * 			잘못땡기면 정보가 틀어진다..
	 * 		=> 새로운 학생이 전학오면 ? 배열을 다시만들어야 함 + 복사해야함
	 * 
	 * 	3) 그래서 나온 개념이 구조체!!
	 * 
	 * 		* 구조체 - 여러개의 자료형의 여러개의 값들을 보관할 수 있음
	 * 				String 값도 보관하고, int 값도 보관하고, double 값도 같이 보관할 수 있도록
	 * 				나만의 배열 같은 자료형을 만들자!! 그게 바로 클래스!!
	 * 
	 * 				Student 라는 클래스로 만들꺼임!! => 객체의 정보를 보관할 그릇과도 같은 이런 클래스들은 = model.vo	 (패키지)
	 * 
	 */
...
//		System.out.println(moon);	// 주소값..
		...
		// 각 영역 "직접 접근"을 해서 내가 원하는 값 대입 => public 이라서 개방돼있음
