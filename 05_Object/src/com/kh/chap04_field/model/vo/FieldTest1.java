 ...
 * >> 변수 구분
 * 
 * - 전역 변수 : 클래스 영역에 바로 선언한 변수 --> 클래스 내에서면 어디서든 적역으로 다 사용 가능하다.(외우는 팁! 전체지역!!)
 * 
...
//		int local;	// 초기화 안하면 안됨. 근데 전역변수는 왜 됨?
		...
		System.out.println(global);	// 초기화 안돼있음. 근데 오류 안남. heap영역애 생성될때 초기화
		
		// 매개변수 출력 => 매개변수는 해당 이 메소드 호출시 반드시 값이 전달될거기 때문에 초기화 안돼있어도됨.
